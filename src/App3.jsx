...
function StudentDashboard({ user }) {
    ...
    const getIncorrectCount = (assignment) => {
        if (assignment.status !== 'Completed' || !assignment.questions) return 'N/A';
        return assignment.questions.filter(q => q.isCorrect === false).length;
    };
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                        ...
                        <tbody>
                            {assignments.map(assignment => (
                                <tr key={assignment.id} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4 font-medium text-gray-900">{assignment.subject}: {assignment.topic}</td>
                                    <td className="px-6 py-4">{assignment.questions?.length || 0}</td>
                ...
                {assignments.length === 0 && <p className="text-center text-gray-500 py-8">You have no assignments.</p>}
            </div>
        </div>
    );
}
...
function AssignmentGenerator({ student, onComplete, parentId }) {
    ...
    const handleGenerate = async (e) => {
        ...
        const includeExplanations = ['Practice', 'Revision', 'Pre-Test'].includes(formData.assignmentPurpose);
