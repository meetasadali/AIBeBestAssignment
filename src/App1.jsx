        ...
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            ...
            if (currentUser) {
                ...
                if (userDoc.exists() && userDoc.data().role === 'parent') {
                    ...
                    userDocRef = doc(db, "students", currentUser.uid);
        ...
        <div className="min-h-screen bg-gray-50 font-sans">
            <header className="bg-white shadow-md">
                <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
                    <div className="text-2xl font-bold text-indigo-600">AI Assignment Hub</div>
            ...
            <main className="container mx-auto p-4 md:p-6">
                {!user ? <AuthScreen /> : (role === 'parent' ? <ParentDashboard user={user} /> : <StudentDashboard user={user} />)}
            </main>
        </div>
    );
}
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                            ...
                            {assignments.map(assignment => (
                                <React.Fragment key={assignment.id}>
                                    <tr className="bg-white border-b hover:bg-gray-50">
                                        <td className="px-6 py-4 font-medium text-gray-900">{assignment.subject}: {assignment.topic}</td>
                                        ...
                                        <td className="px-6 py-4 flex space-x-2">
                                            ...
                                            <button onClick={() => handleDeleteAssignment(assignment.id)} className="text-red-600 hover:underline">Delete</button>
                                        </td>
                                    </tr>
                 ...
                 {assignments.length === 0 && <p className="text-gray-500 text-center py-4">No assignments created yet.</p>}
            </div>
            <Modal isOpen={isStudentModalOpen} onClose={handleCloseStudentModal} title={editingStudent ? "Edit Student Profile" : "Add New Student Profile"}>
                <StudentProfileForm parentId={user.uid} onComplete={handleCloseStudentModal} existingStudent={editingStudent} />
            </Modal>
            <Modal isOpen={isAssignmentModalOpen} onClose={() => setAssignmentModalOpen(false)} title={`Create Assignment for ${selectedStudentForAssignment?.firstName}`}>
                {selectedStudentForAssignment && <AssignmentGenerator student={selectedStudentForAssignment} onComplete={() => setAssignmentModalOpen(false)} parentId={user.uid} />}
            </Modal>
        </div>
    );
}
            ...
            <div className="space-y-4">
                {assignments.map(assignment => (
                    <div key={assignment.id} className="bg-white p-4 rounded-lg shadow-md flex justify-between items-center">
                        <div>
                            <h3 className="font-bold text-lg">{assignment.subject}: {assignment.topic}</h3>
                            <p className="text-sm text-gray-500">Status: {assignment.status}</p>
                        </div>
                        <button onClick={() => setTakingAssignment(assignment)} className="bg-indigo-500 hover:bg-indigo-600 text-white py-2 px-4 rounded-lg">
...
function AssignmentView({ assignment, onBack, userRole }) {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-lg">
            <button onClick={onBack} className="mb-6 text-indigo-600 hover:text-indigo-800 font-semibold">&larr; Back to Dashboard</button>
            <h2 className="text-3xl font-bold mb-2">{assignment.subject}: {assignment.topic}</h2>
            <p className="text-gray-600 mb-8">Complete the questions below.</p>
            ...
            {userRole === 'student' && assignment.status !== 'Completed' && (<button onClick={handleSubmit} disabled={submitting} className="mt-8 w-full bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-4 rounded-lg">{submitting ? 'Submitting...' : 'Submit Assignment'}</button>)}
        </div>
    );
}
...
function StudentProfileForm({ parentId, onComplete, existingStudent }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (existingStudent) {
                ...
                const studentRef = await addDoc(collection(db, "students"), { ...formData, parentId, createdAt: serverTimestamp() });
                const parentRef = doc(db, "users", parentId);
                const parentDoc = await getDoc(parentRef);
                if (parentDoc.exists()) {
                    await updateDoc(parentRef, { managedStudentIds: [...(parentDoc.data().managedStudentIds || []), studentRef.id] });
                }
                setMessage({ type: 'success', text: 'Student added! Auth user creation should be handled by a backend function.' });
            }
...
function AssignmentGenerator({ student, onComplete, parentId }) {
    ...
    const handleGenerate = async (e) => {
        ...
        const assignmentContext = `Assignment Details: Subject: ${formData.subject}. Topics: ${formData.topics}. Purpose: ${formData.assignmentPurpose}. Format: ${formData.assignmentFormat.join(', ')}. Difficulty: ${formData.difficulty}. Number of questions: ${formData.numQuestions}.`;
        const prompt = `Based on this profile: ${studentProfileContext} Create an assignment with these details: ${assignmentContext}. Instructions: Return output as a valid JSON object with a key "questions" which is an array. Each object must have: "id", "type", "text". For MCQs, include "options" array and "correctAnswer". Do not include any markdown or explanatory text outside the JSON.`;
        ...
        try {
            ...
            await addDoc(collection(db, "assignments"), {
                studentId: student.id, parentId, ...formData,
